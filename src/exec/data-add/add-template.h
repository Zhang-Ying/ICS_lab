...
make_helper(concat(add_rm2r_,SUFFIX)){
	...
	if(m.mod==3){
		...
		s=swaddr_read(addr,4)+REG(m.reg);
		result=swaddr_read(addr,4)+REG(m.reg);
		MEM_W(addr,s);
		t=s&0xff;
		t=(t>>1)^(t>>2)^(t>>3)^(t>>4)^(t>>5)^(t>>6)^(t>>7)^(t>>8);
		cpu.eflags.PF=t&0x01;
...
make_helper(concat(add_r2rm_,SUFFIX)){
	...
	char t;
 	if(m.mod==3){
		...
		swaddr_t addr;
		int len=read_ModR_M(eip+1,&addr);
		s=REG(m.reg)+ swaddr_read(addr,4);
        result=REG(m.reg)+swaddr_read(addr,4);
 		REG(m.reg)=s;		
		t=s&0xff;
		t=(t>>1)^(t>>2)^(t>>3)^(t>>4)^(t>>5)^(t>>6)^(t>>7)^(t>>8);
		cpu.eflags.PF=t&0x01;
...
make_helper(concat(add_al_,SUFFIX)){
	...
	if(cpu.eflags.CF==1)
		...
		cpu.eflags.OF=0;
	print_asm("add" str(SUFFIX) " %x,%%%s",imm,REG_NAME(0));
	return DATA_BYTE+1;
}
