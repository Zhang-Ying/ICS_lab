...
uint32_t cache_read_L1(hwaddr_t addr,size_t len){
	...
	if(len==2&&off==(1<<BLOCK_OFFSET)-1)
		return cache_read_L1(addr,1)+(cache_read_L1(addr+1,1)<<8);
	else if(len==4&&off==(1<<BLOCK_OFFSET)-1)
		return cache_read_L1(addr,1)+(cache_read_L1(addr+1,2)<<8)+(cache_read_L1(addr+3,1)<<24);
	else if(len==4&&off==(1<<BLOCK_OFFSET)-2)
		return cache_read_L1(addr,2)+(cache_read_L1(addr+2,2)<<16);
	else if(len==4&&off==(1<<BLOCK_OFFSET)-3)
		return cache_read_L1(addr,2)+(cache_read_L1(addr+2,1)<<16)+(cache_read_L1(addr+3,1)<<24);
	else{
	 	for(count=0;count<WAYS;count++){
...
void cache_write_L1(hwaddr_t addr, size_t len, uint32_t data){
	...
	off=addr&0x3f;	 
	if(len==2&&off==(1<<BLOCK_OFFSET)-1){
		cache_write_L1(addr,1,(data&0xff00)>>8);
		cache_write_L1(addr+1,1,data&0xff);
	}
	else if(len==4&&off==(1<<BLOCK_OFFSET)-1){
		cache_write_L1(addr,1,(data&0xff000000)>>24);
		cache_write_L1(addr+1,2,(data&0x00ffff00)>>8);
		cache_write_L1(addr+3,1,data&0xff);
	}
	else if(len==4&&off==(1<<BLOCK_OFFSET)-2){
		cache_write_L1(addr,2,(data&0xffff0000)>>16);
		cache_write_L1(addr+2,2,data&0xffff);
	}
	else if(len==4&&off==(1<<BLOCK_OFFSET)-3){
		cache_write_L1(addr,2,(data&0xffff0000)>>16);
		cache_write_L1(addr+2,1,(data&0xff00)>>8);
		cache_write_L1(addr+3,1,data&0xff);
	}
	else{
		for(count=0;count<WAYS;count++){ 
